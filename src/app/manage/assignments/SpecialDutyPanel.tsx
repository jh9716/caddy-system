...
export const SpecialDutyPanel = memo(function SpecialDutyPanel({
  ...
  const [selectedKind, setSelectedKind] = useState<DailySpecialKind>("ONE_MAK");
  ...
  async function openModal() {
    setKind(selectedKind === "CHAGEUN" ? "ONE_MAK" : selectedKind);
    setModalOpen(true);
    setQuery("");
    setPaste("");
    setSelected([]);
  ...
  const activeGroup =
    displayGroups.find((group) => group.kind === selectedKind) ||
    displayGroups.find((group) => group.kind === "ONE_MAK") ||
    displayGroups[0];
  const activeKind = activeGroup?.kind || selectedKind;
  const showPlacement =
    isAnchorSpecialKind(activeKind) &&
    ANCHOR_SPECIAL_KINDS.includes(
      activeKind as (typeof ANCHOR_SPECIAL_KINDS)[number]
    );
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
    return (
      <section className="sd-panel">
        ...
        <p className="sd-hint">날짜를 선택하세요.</p>
      </section>
    );
  }
  ...
  return (
    <section className="sd-panel">
      ...
      <div className="sd-kinds" role="tablist" aria-label="특수근무 유형">
        {displayGroups.map((group) => (
          <button
            ...
            className={activeKind === group.kind ? "on" : ""}
            onClick={() => setSelectedKind(group.kind)}
          >
            {group.kind === "CHAGEUN" ? "레거시 찾근" : group.label} {group.count}
          </button>
        ))}
      </div>
        ...
        <div className="sd-body">
          {showPlacement ? (
            <div className="sd-place">
              <div className="sd-place-row">
                <span className="sd-place-title">배치 방식</span>
                <div className="sd-mode" role="group" aria-label="배치 방식">
                  <button
                    type="button"
                    className={placementMode === "AUTO" ? "on" : ""}
                    disabled={busy}
                    onClick={() => void onPlacement("AUTO", protectedTailCount)}
                  >
                    자동
                  </button>
                  <button
                    type="button"
                    className={placementMode === "MANUAL" ? "on" : ""}
                    disabled={busy}
                    onClick={() => void onPlacement("MANUAL", protectedTailCount)}
                  >
                    위치 지정
                  </button>
                </div>
              ...
              {placementMode === "AUTO" ? (
                ...
                <label className="sd-anchor">
                  ...
                  <select
                    value={anchorValue(
                      anchors[activeKind as "ONE_THREE" | "ONE_MAK"]
                    )}
                    disabled={busy || shift1Options.length === 0}
                    onChange={(e) => void onAnchor(activeKind, e.target.value)}
                  >
                    <option value="">
                      {shift1Options.length === 0
                  ...
                  <div className="sd-row">
                    ...
                    <div className="sd-ops">
                      <button
                        type="button"
                        disabled={index === 0}
                        onClick={() => onMoveLocal(activeKind, index, "up")}
                      >
                        위
                      </button>
                      <button
                        type="button"
                        disabled={index === activeGroup.items.length - 1}
                        onClick={() => onMoveLocal(activeKind, index, "down")}
                      >
                        아래
                      </button>
                      <button
                        type="button"
                        className="danger"
                        onClick={() => onDeleteLocal(activeKind, item)}
                      >
                        삭제
                      </button>
                    </div>
          ...
          {dirtyKinds.has(activeKind) ? (
            <button
              ...
              disabled={busy}
              onClick={() => void onSaveKind(activeKind)}
            >
      ...
      {modalOpen ? (
        <div className="sd-modal" role="dialog" aria-modal="true">
          <div className="sd-sheet">
            ...
            <div className="sd-kinds sd-kinds-modal">
            ...
            >
              {busy ? "저장 중…" : `${selected.length}명 한번에 등록`}
            </button>
          </div>
        </div>
      ) : null}
      ...
      <style>{`
        ...
        .sd-title { font-weight: 800; font-size: 0.9rem; }
        .sd-hint {
          margin: 4px 0 0;
          color: #64748b;
          font-size: 0.75rem;
        ...
        .sd-place-title { font-weight: 800; font-size: 0.8rem; }
        .sd-mode {
          display: grid;
        ...
        .sd-tail-input { width: 56px; text-align: center; }
        .sd-window {
          margin-top: 6px;
          font-size: 0.78rem;
          color: #334155;
        }
        .sd-window p { margin: 3px 0; }
        .sd-slot {
          display: block;
          color: #64748b;
          font-weight: 500;
        ...
        .sd-ops button.danger { color: #b91c1c; }
        .sd-modal {
          position: fixed;
          inset: 0;
