...
export default async function ManagePage() {
  ...
  const kpis = [
    { label: "총 캐디", value: totalCaddies, hint: "people" },
    { label: "휴무", value: off, hint: "off" },
    { label: "병가", value: sick, hint: "sick" },
    { label: "장기병가", value: longSick, hint: "long" },
    { label: "당번", value: duty, hint: "duty" },
    { label: "마샬", value: marshal, hint: "marshal" },
  ] as const;

  const kpiIcons: Record<(typeof kpis)[number]["hint"], ReactNode> = {
    people: (
      ...
      <section className="dash-kpi" aria-label="오늘 현황">
        {kpis.map((k) => (
          <article key={k.label} className="dash-kpi-card" data-hint={k.hint}>
            <span className="dash-kpi-icon">{kpiIcons[k.hint]}</span>
        ...
        <div className="dash-glance-row">
          {teamGlance.map((t) => (
            ...
            >
              ...
              <div className="dash-team-total">{t.total}명</div>
            </Link>
          ))}
        </div>
      </section>
