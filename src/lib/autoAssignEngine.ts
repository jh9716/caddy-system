 ...
 * canonical(team-order) leftover로 붙인다. 잘못된 2부 보드(1부 first로
 * 리셋된 상태)는 쓰지 않는다. 큐[0]이 이미 1부 origin이므로 재회전하지 않는다.
 */
function circularHouseFromBoardAndCanonical(
  ...
  for (const caddy of splitCaddyPools(pool).house) push(caddy);
  return ordered;
}
