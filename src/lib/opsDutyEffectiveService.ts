...
import { isOpsDutyRoleKey } from "@/lib/dutyMarshalLeaderParser";
import {
...
export async function loadEffectiveDutyEntries(
  ymd: string,
  client?: PrismaClient
): Promise<DutyExcelEntry[]> {
  const resolved = await resolveEffectiveOpsDuty(ymd, {
    listOverrides: (date) => listDailyOpsDutyOverrides(date, client),
  });
...
export function effectiveOpsDutyJson(result: EffectiveOpsDutyResult) {
  return {
    date: result.date,
