/**
 * Quick MOVE persist: Reservation/Placement + Draft in one transaction.
 * Does not change the general reflow/apply path.
 */
...
export type QuickMoveApplyResult =
  | {
      ok: true;
      changeId: number;
      date: string;
      opsUpdated: boolean;
      preview: NonNullable<Extract<ApplyLiveChangeResult, { ok: true }>["preview"]>;
      draft: DailyBoardDraftRecord;
      timings: {
        computeMs: number;
        persistMs: number;
      };
    }
  | Extract<ApplyLiveChangeResult, { ok: false }>;
...
export async function applyQuickReservationMove(input: {
  ...
  if (preview.changeType !== "MOVE_RESERVATION") {
    return {
      ...
      httpStatus: 400,
      code: "NOT_QUICK_MOVE",
      message: "quick-move는 MOVE_RESERVATION만 저장합니다.",
    };
  }
